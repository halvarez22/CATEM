...
interface FinalMexicoMapProps {
  reports: ReportData[];
  onStateClick: (stateId: string, stateName: string, stateReports: ReportData[]) => void;
}

// Datos GeoJSON reales de México - Estados con formas geográficas precisas
...
const FinalMexicoMap: React.FC<FinalMexicoMapProps> = ({ reports, onStateClick }) => {
  ...
  const stateReportCounts = useMemo(() => {
    ...
    mexicoStatesData.features.forEach(feature => {
    ...
    reports.forEach(report => {
      if (report.state) {
        const feature = mexicoStatesData.features.find(f => f.properties.name === report.state);
        if (feature) {
  ...
  return (
    <div className="bg-[#2d3748] p-6 sm:p-8 rounded-lg shadow-2xl animate-fade-in">
      ...
      <div className="mt-6">
        ...
        {mexicoStatesData.features.filter(feature => stateReportCounts[feature.properties.id] > 0).length === 0 && (
