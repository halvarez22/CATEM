...
import { mexicoGeoJSON } from '../data/mexicoGeoJSON';
...
interface LeafletMexicoMapProps {
  reports: ReportData[];
  onStateClick: (stateId: string, stateName: string, stateReports: ReportData[]) => void;
}

const LeafletMexicoMap: React.FC<LeafletMexicoMapProps> = ({ reports, onStateClick }) => {
  ...
  const filteredFeatures = useMemo(() => {
    if (!searchTerm) return mexicoGeoJSON.features;
    
    return mexicoGeoJSON.features.filter(feature => 
  ...
  const onEachFeature = (feature: any, layer: any) => {
    ...
    layer.bindPopup(`
      <div class="p-2">
        ...
        <p class="text-sm text-gray-600">${denuncias} denuncia${denuncias !== 1 ? 's' : ''}</p>
        <p class="text-xs text-gray-500">${getColorDescription(denuncias)}</p>
      </div>
    `);
    ...
    layer.on({
      ...
      mouseout: (e: any) => {
        ...
        e.target.setStyle({
          weight: selectedState === stateId ? 3 : 2,
  ...
  return (
    <div className="bg-[#2d3748] p-6 sm:p-8 rounded-lg shadow-2xl animate-fade-in">
      ...
      {/* Mapa Real de México con Leaflet */}
